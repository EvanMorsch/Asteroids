//make pseudo-interrupt functions to be called along with event handler
...
class _mouse {
	constructor() {
		...
		this.buttons = [];
		this.dragLocations = [];
		this.loc = new Position2D(0,0)//shortened so it wont interfere with getter
		this.wheel = {
			position: 0,
	...
	mouseMove(e) {
		...
		if (this.buttons.some(a=>a.state)) {
			...
			this.dragLocations.push(new Position2D(this.location.x,this.location.y));
			this.drag = true;
		}
	}
	wheelMove(e) {
